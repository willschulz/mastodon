...
module Paginable
  ...
  included do
    ...
    scope :paginate_by_max_id_fav, ->(limit, max_id = nil, since_id = nil) {
      ...
      query = joins(:status_stat).where(StatusStat.arel_table[:favourites_count].in(10..70)).reorder(Arel::Nodes::NamedFunction.new('concat', [StatusStat.arel_table[:favourites_count], arel_table[:id]]).desc).limit(20)
      query = query.where(arel_table[:id].lt(max_id)) if max_id.present?
      query = query.where(arel_table[:id].gt(since_id)) if since_id.present?
      query = query
      query
