...
module Paginable
  ...
  included do
    ...
    scope :paginate_by_max_id_fav, ->(limit, max_id = nil, since_id = nil) {
      ...
      # query = query.where(arel_table[:id].gt(since_id)) if since_id.present?
      # query
      query = joins(:status_stat).order(StatusStat.arel_table[:favourites_count].desc, arel_table[:id].desc).limit(3)#this is where we go back #remember to add back  after [:id]
      query = query.where(arel_table[:id].lt(max_id)) if max_id.present?
      query = query.where(arel_table[:id].gt(since_id)) if since_id.present?
      query
    }
